...
<head>
    ...
    <title>اختبار الدرس الأول | الحقيبة التعليمية</title>
    ...
    <style>
        ...
        .q-section { margin-bottom: 25px; padding: 15px; border-right: 5px solid #1a73e8; background: #fcfcfc; }
        ...
        input[type="radio"]:checked + label { background: #1a73e8; color: white; border-color: #1a73e8; }
        .btn-submit { width: 100%; padding: 15px; background: #27ae60; color: white; border: none; border-radius: 8px; font-size: 1.1rem; cursor: pointer; margin-top: 20px; }
...
<body>
    <div class="container">
        <div class="quiz-container" id="quiz-ui">
            <div id="timer-box" style="text-align: center; font-size: 1.5rem; font-weight: bold; color: #e74c3c; margin-bottom: 15px;">
    ⏱️ الوقت المتبقي: <span id="time-left">05:00</span>
</div>
            <h2 style="text-align: center;">🧠 اختبار: الجهاز التنفسي</h2>
            <form id="qForm">
                
                <h4 style="color: #1a73e8;">أولاً: اختر الإجابة الصحيحة</h4>
                ...
                <h4 style="color: #e67e22; margin-top: 30px;">ثانياً: ضع علامة صح أو خطأ</h4>
                ...
                <button type="button" onclick="calculateResult(1)" class="btn-submit">عرض النتيجة وفتح الدرس التالي</button>
            </form>
        </div>
        ...
        <div id="result-box" class="quiz-container">
            <h1 id="score-text"></h1>
            <p id="feedback"></p>
            <a href="quizzes.php" class="btn-submit" style="text-decoration: none; display: block;">العودة لصفحة الاختبارات</a>
        </div>
    </div>

    <script>
        let timeLeft = 300; // 300 ثانية يعني 5 دقائق
...
const timer = setInterval(() => {
    ...
    // إضافة صفر لو الثواني أقل من 10 (عشان يبقى شكلها 05:09)
    seconds = seconds < 10 ? '0' + seconds : seconds;
    timerDisplay.innerHTML = `${minutes}:${seconds}`;

    if (timeLeft <= 0) {
        clearInterval(timer);
        alert("⚠️ انتهى الوقت! سيتم تصحيح إجاباتك الآن.");
        calculateResult(1); // استدعاء دالة النتيجة أوتوماتيك (غير الرقم حسب الدرس)
    }
    timeLeft--;
}, 1000);

// تعديل بسيط على دالة النتيجة عشان توقف العداد لما الطالب يخلص بدري
function calculateResult(lessonID) {
    clearInterval(timer); // وقف العداد أول ما يدوس إرسال
    ...
    if(score >= 7) {
        document.getElementById('feedback').innerText = "رائع! نجحت قبل انتهاء الوقت.";
        let lastDone = localStorage.getItem('quizCompleted') || 0;
        if(lessonID > lastDone) localStorage.setItem('quizCompleted', lessonID);
    } else {
        document.getElementById('feedback').innerText = "للأسف لم تنجح، حاول مرة أخرى.";
    }
}
    </script>
